...
meanFitness <- fread('meanFitness_20220316.csv', header=TRUE)
significantFitness <- fread('significant_genes_grouped_Metabolism_filtered_20220413.csv', header=TRUE)
table1 <- fread('Table1_20220328.csv', header=TRUE) %>% 
...
overallFitPlot <- function(meanFitX=meanFitness$mean_rpomAlone, meanFitY=meanFitness$mean_vib, SigFitY_X, SigFitY_Y, Sig_diff, SigTableX, SigTableY, xLab, yLab, ycol){
    ...
    text(0.5, 1.8, paste('Fitness Increased=', length(Sig_diff[Sig_diff>0 & !is.na(Sig_diff)])), cex=0.6)
    text(0.5, 1.7, paste('Fitness Decreased=', length(Sig_diff[Sig_diff<0 & !is.na(Sig_diff)])), cex=0.6)
}
...
#pdf('fitnessOverview20220413.pdf', height=3, width=9,useDingbats=FALSE)
par(mfrow=c(1,4), mar=c(5,4,4,1))

overallFitPlot(meanFitness$mean_rpomAlone, meanFitness$mean_vib, vibSig$mean_rpomAlone, vibSig$mean_vib, vibSig$Vib_diff, vibSigTable$mean_rpomAlone, vibSigTable$mean_vib, 'R. pomeroyi Alone','+ Vibrio', vibColor)


overallFitPlot(meanFitness$mean_rpomAlone, meanFitness$mean_mari, mariSig$mean_rpomAlone, mariSig$mean_mari, mariSig$Mari_diff, mariSigTable$mean_rpomAlone, mariSigTable$mean_mari, 'R. pomeroyi Alone','+ Marivivens', mariColor)

overallFitPlot(meanFitness$mean_rpomAlone, meanFitness$mean_CC, commSig$mean_rpomAlone, commSig$mean_CC, commSig$CC_diff,commSigTable$mean_rpomAlone, commSigTable$mean_CC, 'R. pomeroyi Alone','+ Complete Community', compCommColor)
#dev.off()
```
...
#pdf('log2FC_Whist_20220413_selectRange.pdf', width=5, height=5)
hist(vibSig$Vib_fold_diff, seq(-2.5, 6.6, length.out = 150), col=vibColor, xlim=c(-6.5, 6.5), ylim=c(0,30), border='white', xlab='log2_FC Rp+V', main='')
hist(mariSig$Mari_fold_diff, seq(-2.5, 6.6, length.out = 150), col=mariColor, xlim=c(-6.5, 6.5), ylim=c(0,30), border='white', xlab='log2_FC Rp+M', main='')
hist(commSig$CC_fold_diff, seq(-6.6, 6.6, length.out = 150), col=compCommColor, xlim=c(-6.5, 6.5), ylim=c(0,30), border='white', xlab='log2_FC Rp+V+M', main='')
#dev.off()
...
glycolate_mean$mean_OD[glycolate_mean$Organism!='Blank' & glycolate_mean$Condition=='Malate'] <- glycolate_mean$mean_OD[glycolate_mean$Organism!='Blank' & glycolate_mean$Condition=='Malate'] - glycolate_mean$mean_OD[glycolate_mean$Organism=='Blank' & glycolate_mean$Condition=='Malate']

attach(glycolate_mean)
rpomGlycolate <- Organism=='Rpom' & Condition=='Glycolate'
rpomMal <- Organism=='Rpom' & Condition=='Malate'
vibGlycolate <- Organism=='Vib' & Condition=='Glycolate'
vibMal <- Organism=='Vib' & Condition=='Malate'
mariGlycolate <- Organism=='Mari' & Condition=='Glycolate'
mariMal <- Organism=='Mari' & Condition=='Malate'
...
plot(Time[rpomMal], mean_OD[rpomMal], type='n',  las=1, xlab='Time(Hr)', ylab='OD600', main='Growth on Malate', bty='n', ylim=c(0, 0.3),cex=1.5, col='gray80')
OD(rpomMal, rpomColor)
OD(vibMal, vibColor)
OD(mariMal, mariColor)
legend(2, 0.2, legend=c('R. pomeroyi WT', 'Vibrio', 'Marivivens'), col=c(rpomColor, vibColor, mariColor), bty='n', pch=16, cex=0.6)
#dev.off()
...
spermidine_OD <- fread('Data/Spermidine_growth_CBS.csv') %>%  melt(., id=('Time'), variable.name='Well', value.name='OD600')

spermidine_layout <- fread('Data/Spermidine_growth_CBS_platelayout.csv')


spermidine_growthData <- inner_join(spermidine_OD, spermidine_layout , by='Well')


spermidine_mean <- meanCells(spermidine_growthData)

# Blank transformed data

spermidine_mean$mean_OD[spermidine_mean$Organism!='Blank' & spermidine_mean$Condition=='Spermidine'] <- spermidine_mean$mean_OD[spermidine_mean$Organism!='Blank' & spermidine_mean$Condition=='Spermidine'] - spermidine_mean$mean_OD[spermidine_mean$Organism=='Blank' & spermidine_mean$Condition=='Spermidine']

attach(spermidine_mean)
rpomSpermidine <- Organism=='Rpom' & Condition=='Spermidine'
vibSpermidine <- Organism=='Vib' & Condition=='Spermidine'
mariSpermidine <- Organism=='Mari' & Condition=='Spermidine'

#pdf('spermidineGrowth.pdf')
plot(Time[rpomSpermidine], mean_OD[rpomSpermidine], type='n',  las=1, xlab='Time(Hr)', ylab='OD600', main='Growth on Spermidine', bty='n', ylim=c(0, 0.2),cex=1.5, col='gray80')
OD(rpomSpermidine, rpomColor)
OD(vibSpermidine, vibColor)
OD(mariSpermidine, mariColor)
#dev.off()
detach()
```
```{r}
...
plot(Time[rpomArg], mean_OD[rpomArg], type='n',  las=1, xlab='Time(Hr)', ylab='OD600', main='Growth on Arginine', bty='n', ylim=c(0, 0.2),cex=1.5, col='gray80')
...
plot(Time[rpomGln], mean_OD[rpomGln], type='n',  las=1, xlab='Time(Hr)', ylab='OD600', main='Growth on Glutamine', bty='n', ylim=c(0, 0.2),cex=1.5, col='gray80')
...
GluRibose_mean$mean_OD[GluRibose_mean$Organism!='Blank' & GluRibose_mean$Condition=='Ribose'] <- GluRibose_mean$mean_OD[GluRibose_mean$Organism!='Blank' & GluRibose_mean$Condition=='Ribose'] - GluRibose_mean$mean_OD[GluRibose_mean$Organism=='Blank' & GluRibose_mean$Condition=='Ribose']
...
plot(Time[rpomGlu], mean_OD[rpomGlu], type='n',  las=1, xlab='Time(Hr)', ylab='OD600', main='Growth on Glutamate', bty='n', ylim=c(0, 0.3),cex=1.5, col='gray80')
...
rpomRib <- Organism=='Rpom' & Condition=='Ribose' & Time < 60
vibRib <- Organism=='Vib' & Condition=='Ribose' & Time < 60
mariRib <- Organism=='Mari' & Condition=='Ribose' & Time < 60

#pdf('RiboseGrowth.pdf')
plot(Time[rpomRib], mean_OD[rpomRib], type='n',  las=1, xlab='Time(Hr)', ylab='OD600', main='Growth on Ribose', bty='n', ylim=c(0, 0.3),cex=1.5, col='gray80')
OD(rpomRib, rpomColor)
OD(vibRib, vibColor)
OD(mariRib, mariColor)
#dev.off()
