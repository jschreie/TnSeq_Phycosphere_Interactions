...
stdev <- as.numeric(rep(NA, spoListLength))
lowerCL_adj <- as.numeric(rep(NA, spoListLength))
upperCL_adj <- as.numeric(rep(NA, spoListLength))
...
expansion_fitnessCalc <- mariExpansion

initial_treatmentColumns_meanFitnessCalc <- initial
treatmentColumns_meanFitnessCalc <- mari

outputName <- 'mariFitness_20220531.csv'
...
for(i in 1:nrow(spoFunction)){
    ...
    alpha <- 0.05
    sdev <- sd(meanW, na.rm=TRUE)
    lwrCL <- quantile(meanW, alpha/2)
    uprCL <- quantile(meanW, 1-alpha/2)
    
    alpha_adj <- 0.05/nrow(spoFunction)
    lwrCL_adj <- quantile(meanW, (alpha_adj/2))
    uprCL_adj <- quantile(meanW, 1-alpha_adj/2)
            
    meanFitnessTable[i] <- meanFit
    lowerCL[i] <- lwrCL
    upperCL[i] <- uprCL
    stdev[i] <- sdev
    lowerCL_adj[i] <- lwrCL_adj
    upperCL_adj[i] <- uprCL_adj
    spoFitnessTable[i] <- spoFit
    naCount[i] <- meanWNA
}

fitness <- data.frame(matrix(ncol=8, nrow=nrow(spoFunction)))
colnames(fitness) <- c('mean', 'lwrCL', 'uprCL', 'stdev', 'lwrCL_adj', 'uprCL_adj', 'spo', 'naCount')

fitness$mean <- round(meanFitnessTable, 3)
fitness$lwrCL <- round(lowerCL, 3)
fitness$uprCL <- round(upperCL, 3)
fitness$stdev <- round(stdev, 3)
fitness$lwrCL_adj <- round(lowerCL_adj, 3)
fitness$uprCL_adj <- round(upperCL_adj, 3)
